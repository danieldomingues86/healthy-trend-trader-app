...
- Trading Rubric grades opportunity quality across the 100-point rubric: Grade A+ (Rare Trade com Confluência Máxima, score 95-100 com excelência em todos os critérios críticos), Grade A (High Quality, score 85-94 com critérios críticos válidos), Grade B (Good Edge, 70-84), Grade C (Small Edge, 50-69), e Grade D (No Edge / medíocre, <50 ou falha em gate crítico). Grade D é um bloqueio automático com risco nominal zero (0%). A política de risco resolve o Risk Budget nominal de cada Grade.
- Filosofia de Setup Único e Nomenclatura: No Healthy Trend Trader, o setup operacional é sempre o mesmo (Trend Following sistemático); o que varia de uma operação para outra é a qualidade e confluência de edges da oportunidade. Portanto, nunca se deve referir a "diferentes setups". Na tela de Política de Risco, o Bloco 2 é denominado "Quanto arriscar por qualidade do trade (Notas A, B, C e D)" e o Bloco 3 é denominado "Pesos de cada edge", comunicando de forma clara para traders leigos.
...
- Desafio Grade A/A+ (Modo Desapego): evaluates 20 trades strictly classified as Grade A or Grade A+ executed while the challenge is active. It verifies execution compliance against the authorized executable risk within a strict tolerance of `±0.02pp`. It does not alter position sizing calculations, nor does it count legacy or non-qualifying trades.
- Emerging Leaders (Líderes Emergentes): 6-dimension scoring engine (Relative Strength 25%, RS Acceleration 15%, 52-Week High Proximity 20%, Correction Resilience 15%, Recovery Velocity 15%, Sector Alignment 10%). Discovery is not execution: high-scoring assets feed the Watchlist Opportunity Pool and require technical base contraction and confirmed trigger before any entry is planned.
- Behavioral Asset Blacklist (`assetblacklist`): protects the trader by enforcing personal/behavioral risk guardrails that block orders and planning on toxic, illiquid or forbidden instruments.
- Trade Anatomy (`tradeanatomy`): post-trade statistical analysis module evaluating win rate, R-multiple payoff, duration, and setup attribution.
- Central de Conhecimento / "Explore o Software" showcase standard: each tool card in the manual showcase uses a conceptual / analogical cover image (`assets/manual-knowledge/<id>.jpg` at 1.91:1 ratio, dark emerald and gold aesthetic) that visually answers "what this tool stands for" rather than raw UI screenshots. This prevents visual obsolescence when internal screens evolve.
- Navigation structure: managed by `frontend/sidebar-navigation.js` and encompasses 30+ pages organized into Daily Operation, Market Analysis, Portfolio & Risk, Data & Performance, Mindset & Zen, and Footer Utilities.
- Fundamental Analysis (`fundamentals`): integra ações B3 (CVM Dados Abertos + Fundamentus) e BDRs internacionais (banco de demonstrações contábeis SEC `backend/data/fundamentals-bdr.json`). O Score Fundamentalista (0 a 10) segue a metodologia de 6 dimensões ponderadas: Rentabilidade (28%), Consistência de Lucros (22%), Crescimento (16%), Endividamento (18%), Valuation (10%) e Dividendos (6%). O princípio inviolável é: **dado ausente não é zero**. Quando métricas ou históricos contábeis anuais não estão disponíveis, o sistema repondera os componentes disponíveis e expõe uma métrica explícita e separada de **Cobertura dos Dados** (ex: 78% em CEAB3) com alertas claros de dados parciais. A tela é 100% explicável e auditável ("Como esta nota foi calculada"): a soma das contribuições matemáticas exibidas em cada dimensão resulta identicamente no score final exibido, acompanhada de drill-down de indicadores, Company Snapshot com Market Cap humanizado (`formatLargeNumber`: mil, mi, bi, tri), Régua Horizontal precisa de 5 patamares (*RUIM, FRACO, MÉDIO, BOM, EXCELENTE* — sem velocímetros) e Raio-X Fundamentalista agrupado por categorias econômicas com badges de contexto baseadas em regras de negócio reais.
...
- Exportação do Diário do Trader (`journal-export-model.js`): Permite exportar os registros do diário estruturados para estudo e análise em LLMs (ChatGPT, Claude, Gemini). Suporta múltiplos períodos (mês atual, dia atual, últimos 30 dias, últimos 3 meses, ano, customizado, todo o histórico), seleção de conteúdo (técnico, emocional, ambos) e formatos (Markdown, Texto, JSON, CSV). O modo "Preparar para IA" gera formatação semântica limpa por blocos de data com prompt de instruções para o modelo analisar correlações entre contexto, execução, plano e emoções. A exportação consulta exclusivamente os dados já existentes em memória (`JournalV2Model`) sem duplicar armazenamento.
