...
test('provider combina CVM e cache Fundamentus no contrato normalizado',async()=>{const cache=JSON.parse(await fs.readFile(DATA_FILE,'utf8'));const symbol=Object.keys(cache.companies)[0];assert.ok(symbol);clearFundamentalsCache();const result=await fetchFundamentals(` ${symbol.toLowerCase()} `);assert.equal(result.ticker,symbol);assert.match(result.provider,/CVM Dados Abertos/);assert.ok(result.fetchedAt);assert.ok(result.company)});
