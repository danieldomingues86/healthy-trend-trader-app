(function () {
  ...
  const software = [
    ...
    ['emergingleaders','Emerging Leaders','Descubra a próxima geração de líderes.','manual-emergingleaders','Identifique ações com alta Força Relativa, aceleração e proximidade da máxima durante correções. Acompanhe clusters setoriais antes da entrada.'],
  ...
  function guide(id) {
    ...
    const additionalGuides = {
      emergingleaders: [
        'O Emerging Leaders Scan combina 6 dimensões em um Score único de 0 a 100: Força Relativa (25%), Aceleração de RS (15%), Proximidade da Máxima de 52S (20%), Resiliência na Correção (15%), Força de Recuperação (15%) e Liderança Setorial (10%).',
        'Use esta ferramenta para descobrir quais ações estão sustentando preço enquanto a maioria do mercado cai. Veja os setores com maior concentração de força (Clusters Fortes) e selecione os melhores candidatos.',
