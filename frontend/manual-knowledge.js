(function () {
  ...
  const software = [
    ...
    ['riskpolicy','Política de Risco','Governança e calibração de risco.','manual-riskpolicy','Defina os limites de risco nominal para cada Grade (A+, A, B, C e D), configure perfis operacionais e calibre os pesos da Trading Rubric.'],
    ['couragechallenge','Desafio A/A+','Treinamento do Modo Desapego.','manual-courage','Acompanhe sua consistência na execução de operações Grade A e A+ no tamanho nominal definido pela sua Política de Risco.'],
  ...
  const faq = [
    ...
    ['Como o Grade da Rubric define o risco-base?','A Trading Rubric avalia os 6 critérios na escala de 100 pontos e determina a Grade (A+, A, B, C ou D). O sistema busca dinamicamente na Política de Risco o percentual configurado para essa Grade (ex: B = 0,20%, A = 0,40%, A+ = 0,50%). A cadeia inteira (Grade → Risco-base → Risco financeiro → Position Size) é recalculada instantaneamente a cada clique.'],
    ['O que acontece se meu setup for Grade B em vez de Grade A?','O sistema aplica o risco-base configurado para B (ex: 0,20% em vez de 0,40%), reduzindo a quantidade de ações pela metade (ex: de 1.600 para 800 ações com stop de R$ 2,50 em R$ 1.000.000). Além disso, o trade não pontua no Desafio A/A+.'],
    ['O Desafio A/A+ altera o tamanho da minha posição?','Não. O Desafio A/A+ nunca altera o cálculo de Position Sizing. Ele apenas inspeciona o plano para verificar se você executou a operação no risco-base nominal estipulado na sua Política de Risco para aquele Grade (A ou A+), recompensando a coragem disciplinada.'],
  ...
  function guide(id) {
    ...
    const additionalGuides = {
      newtrade: [
        ...
        'Risco-Base Dinâmico por Grade: A classificação na Rubric busca dinamicamente o risco configurado na sua Política de Risco para o Grade obtido (Grade B = 0,20%, Grade A = 0,40%, Grade A+ = 0,50%). Qualquer alteração de critério na Rubric recalcula imediatamente toda a cadeia: Grade → Risk Policy → Risco-base → Risco financeiro → Position Size.',
        ...
        'Integração com o Desafio A/A+: Operações com classificação Grade A ou A+ qualificam automaticamente para o Desafio A/A+ em Modo Desapego, inspecionando se a execução respeitou o risco nominal configurado na política.'
      ],
      riskpolicy: [
        'A Política de Risco centraliza os perfis operacionais (Política Padrão e Risk Ramp-Up), a calibração de risco nominal para cada Grade (A+, A, B, C e D) e os pesos da Trading Rubric.',
        'Cards de Grades: Exibe e permite editar os percentuais nominais associados a cada Grade. Por padrão: Grade A+ (0,50%), Grade A (0,40%), Grade B (0,20%), Grade C (0,10%) e Grade D (0,00% — bloqueio automático).',
        'Conexão Direta com Novo Trade: Qualquer alteração salva na Política de Risco atualiza instantaneamente a resolução de risco do Novo Trade e as validações de conformidade do Desafio A/A+.'
      ],
      couragechallenge: [
        'O Desafio A/A+ é um módulo de treinamento psicológico (Modo Desapego) projetado para consolidar a coragem e a disciplina de assumir o risco correto nos melhores setups do método.',
        'Elegibilidade Estrita: Apenas setups classificados como Grade A ou Grade A+ participam do desafio. Trades Grade B, C e D são ignorados para proteger o foco e evitar overtrading.',
        'Inspeção sem Interferência: O desafio não altera o cálculo do Position Sizing; ele apenas inspeciona o trade registrado contra o risco-base nominal estipulado na sua Política de Risco para aquele Grade (A ou A+).'
      ],
      emergingleaders: [
