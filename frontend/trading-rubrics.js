(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.TradingRubrics=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  const DEFAULT_POLICY={
    criteria:[
      {key:'marketCycle',label:'Ciclo de Mercado · saudável?',weight:1},
      {key:'trendQuality',label:'Diário e Contexto · preço > MM20 e MM200',weight:1},
      {key:'relativeStrength',label:'RS Rank (Força Relativa)',weight:1},
      {key:'fundamentalScore',label:'Score Fundamentalista · Grade',weight:1},
  ...
  function normalizePolicy(policy={}){const merged={...clone(DEFAULT_POLICY),...policy};const savedCriteria=policy.criteria||[];merged.criteria=DEFAULT_POLICY.criteria.map(base=>{const saved=savedCriteria.find(c=>c.key===base.key||(base.key==='fundamentalScore'&&c.key==='sector'))||{};return{...base,weight:Number(saved.weight??base.weight)}});merged.ratingScale={...DEFAULT_POLICY.ratingScale,...(policy.ratingScale||{})};merged.grades=(policy.grades||DEFAULT_POLICY.grades).filter(g=>g.grade!=='A').map(g=>({...g,minScore:Number(g.minScore),riskPct:Number(g.riskPct)})).sort((a,b)=>b.minScore-a.minScore);return merged}
  function riskBaseOptions(policy={}){return [...new Set(normalizePolicy(policy).grades.map(grade=>grade.riskPct).filter(Number.isFinite))].sort((a,b)=>a-b)}
  ...
  return{DEFAULT_POLICY,normalizePolicy,riskBaseOptions,calculateRubric,calculatePositionSizing,calculateOngoingRisk,validatePortfolio};
});
