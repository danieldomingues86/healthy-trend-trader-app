(function () {
  ...
  function fillFromTrade(select) {
    ...
    field('ticker').value = (trade.ticker || trade.asset || '').toUpperCase();
    field('setup').value = (trade.setup || trade.metadata?.setup || '').toUpperCase();
    field('market').value = (trade.market || '').toUpperCase();
    field('timeframe').value = (trade.timeframe || trade.metadata?.timeframe || '').toUpperCase();
  ...
  root.addEventListener('input', event => {
    if (event.target.matches('[data-search]')) { state.query = event.target.value; const pos = event.target.selectionStart; render(); const input = root.querySelector('[data-search]'); input.focus(); input.setSelectionRange(pos, pos); return; }
    if (event.target.matches?.('.mb-form input[name="ticker"], .mb-form input[name="setup"], .mb-form input[name="timeframe"], .mb-form input[name="market"]')) {
  ...
  root.addEventListener('click', event => {
    ...
    if (button.hasAttribute('data-new')) { state.section = 'records'; openModal(); return; }
    ...
    if (button.hasAttribute('data-clear-filters')) { state.section = 'records'; state.filter = 'Todos'; state.query = ''; state.selected = null; render(); return; }
    if (button.dataset.edit) { openModal(button.dataset.edit); return; }
    if (button.dataset.select) { state.section = 'records'; state.selected = button.dataset.select; render(); return; }
    if (button.dataset.filter) { state.section = 'records'; state.filter = button.dataset.filter; state.selected = null; render(); return; }
    if (button.dataset.related) { state.section = 'records'; state.filter = 'Todos'; state.query = button.dataset.related; state.selected = null; render(); return; }
  ...
  masteredRoot.addEventListener('click', event => { const button = event.target.closest('button'); if (!button) return; state.section = 'records'; if (button.dataset.openMistake) state.selected = button.dataset.openMistake; window.go?.('mistakesbook'); render(); });
