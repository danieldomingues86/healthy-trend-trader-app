(() => {
  ...
  function render() {
    ...
    const rareTrades = summary.trades.filter(trade => trade.rubric_grade === 'A').length;
    ...
    root.innerHTML = `<main class="weekly-review-page">
      ...
      <section class="weekly-metrics"><article><small>RESULTADO REALIZADO</small><strong class="${summary.total >= 0 ? 'positive' : 'negative'}">${money(summary.total)}</strong><span>${tradeCount} trade(s) encerrado(s)</span></article><article><small>TAXA DE ACERTO</small><strong>${percent(winRate)}</strong><span>${summary.wins} ganho(s) · ${tradeCount - summary.wins} perda(s)</span></article><article><small>EXECUÇÃO NO DIÁRIO</small><strong>${summary.execution == null ? '—' : `${summary.execution.toFixed(1).replace('.', ',')} / 10`}</strong><span>${summary.records.length} registro(s) no período</span></article><article><small>RARE TRADES · A</small><strong>${rareTrades}/${tradeCount || '—'}</strong><span>${tradeCount ? percent(rareTrades / tradeCount * 100) : 'Sem trades encerrados'}</span></article></section>
